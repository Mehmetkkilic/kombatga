...
function animate() {
    ...
    if (gameActive && p1 && p2) {
        ...
        // --- YENİ FİZİKSEL ÇARPIŞMA (İÇİNDEN GEÇMEME) ---

        // Sadece ikisi de hayattaysa
        ...
        if (!p1.dead && !p2.dead) {
            ...
            // Çarpışma eşiği (Gövde kalınlığına göre)

            const collisionThreshold = 1.5; 

            // Eğer çok yakınlarsa VE yükseklikleri benzerse (Zıplamıyorsa)

            if (dist < collisionThreshold && Math.abs(dy) < 2.0) {

                // Birbirlerini itmeleri lazım

                const overlap = collisionThreshold - dist;

                const pushDir = dx > 0 ? 1 : -1; // P1, P2'nin sağında mı solunda mı?

                // İki oyuncuyu da zıt yöne it (Eşit kuvvetle)

                p1.mesh.position.x += (overlap / 2) * pushDir;

                p2.mesh.position.x -= (overlap / 2) * pushDir;

            }

        }

        // Vuruş (Hitbox) Kontrolü
