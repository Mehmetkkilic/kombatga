...
function initPeer() {
    ...
    peer.on('connection', c => {
        conn = c; IS_HOST = true;
        conn.on('open', () => {
            conn.on('data', d => { if (d.type === 'input') { handleInput(IS_HOST ? p2 : p1, d.key, d.isDown); } });
            VS_AI = false; IS_ONLINE = true; startGame(); conn.send({ type: 'START' });
        });
    });
}
window.joinGame = function() {
    const id = document.getElementById('friend-id').value.trim(); if (!id) return alert("ID YOK");
